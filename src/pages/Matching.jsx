    ...
    <div className={`${p.bg} ${p.border} border rounded-[28px] p-5 transition-all hover:scale-[1.01]`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-2xl font-light text-white">{match.score}%</span>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-center mb-4">
        ...
        <div className="text-right">
          ...
          <p className="text-white/50 text-[11px]">{v.prix_vente > 0 ? Number(v.prix_vente).toLocaleString('fr-FR') + ' €' : '—'} · {v.secteur || '—'}</p>
...
export default function Matching() {
  ...
  async function handleRelancer() {
    ...
    if (error) { setMsg({ ok: false, text: 'Erreur : ' + error }); setRunning(false); return }
    ...
    <div className="p-6 md:p-10">
      ...
      <div className="flex flex-wrap gap-3 items-center mb-6">
        ...
        <div className="liquid-glass rounded-2xl px-4 py-2 flex items-center gap-3 border border-white/10 flex-1 min-w-[200px]">
          ...
          <input
            type="range" min="20" max="90" value={minScore}
